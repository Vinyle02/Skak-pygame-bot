...
charDict = {0:"a",1:"b",2:"c", 3:"d",4:"c",5:"e",6:"f",7:"h"}
...
class Chess(object):
            ...
            while x>0:
                if(x==8):
                    if(chr(i)=='a' or chr(i)=='h'):
                        cur = self.piece_location[chr(i)][x][0] = "black_rook"
                    elif(chr(i)=='b' or chr(i)=='g'):
                        cur = self.piece_location[chr(i)][x][0] = "black_knight"
                    elif(chr(i)=='c' or chr(i)=='f'):
                        cur = self.piece_location[chr(i)][x][0] = "black_bishop"
                    elif(chr(i)=='d'):
                        cur = self.piece_location[chr(i)][x][0] = "black_queen"
                    elif(chr(i)=='e'):
                        cur = self.piece_location[chr(i)][x][0] = "black_king"
                    black_pieces.append(cur)
                    black_pieces_loc.append((chr(i),x))
                elif(x==7):
                    cur = self.piece_location[chr(i)][x][0] = "black_pawn"
                    black_pieces.append(cur)
                    black_pieces_loc.append((chr(i),x))
                elif(x==2):
                    cur = self.piece_location[chr(i)][x][0] = "white_pawn"
                    white_pieces.append(cur)
                    white_pieces_loc.append((chr(i),x))
                elif(x==1):
                    if(chr(i)=='a' or chr(i)=='h'):
                        cur = self.piece_location[chr(i)][x][0] = "white_rook"
                    elif(chr(i)=='b' or chr(i)=='g'):
                        cur = self.piece_location[chr(i)][x][0] = "white_knight"
                    elif(chr(i)=='c' or chr(i)=='f'):
                        cur = self.piece_location[chr(i)][x][0] = "white_bishop"
                    elif(chr(i)=='d'):
                        cur = self.piece_location[chr(i)][x][0] = "white_queen"
                    elif(chr(i)=='e'):
                        cur = self.piece_location[chr(i)][x][0] = "white_king"
                    white_pieces.append(cur)
                    white_pieces_loc.append((chr(i),x))
                x = x - 1
    ...
    def AI_test(self):
        ...
        if(self.turn["black"]):
            for i, piece in enumerate(black_pieces):
                x, y = self.piece_location[black_pieces_loc[i][0]][black_pieces_loc[i][1]][2]
                legal_moves = self.possible_moves(piece,[x,y])
                if legal_moves:
    ...
    def choose_random_piece(self):
        ...
        if(self.turn["white"]):
            square = [white_pieces[rand],white_pieces_loc[rand][0],white_pieces_loc[rand][1]]
        else:
            square = [black_pieces[rand], black_pieces_loc[rand][0], black_pieces_loc[rand][1]]
        return square
    ...
    def move_piece(self, turn):
        ...
        #square[1] = black_pieces_loc[black_pieces[5]][0]
        #square[1] = black_pieces_loc[black_pieces[5]][1]
        ...
        if square:
            ...
            if self.moves:
                move = random.randrange(0,len(self.moves))

                if (self.turn["white"]):
                    white_pieces_loc[rand] = (chr(self.moves[move][0]+97),8-self.moves[move][1])
                else:
                    black_pieces_loc[rand] = (chr(self.moves[move][0]+97), 8-self.moves[move][1])
                x, y = self.moves[move]
            else:
                x, y = (-1,-1)
            ...
            if(piece_color == turn):
                ...
                if(self.piece_location[columnChar][rowNo][0]):
    ...
    def validate_move(self, destination):
        print("Validating " + str(destination))
